...
const sendBookingConfirmationEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await sendEmail({
      ...
      <p>Enjoy the show! </p>
      <p>Thanks for booking with us!<br />- jusTickets Team</p>
    </div>`
    })
  }
)
...
const sendShowReminders = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const results = await step.run('send-all-reminders', async () => {
      return await Promise.allSettled(
        ...
        <p>is scheduled for <strong> ${new Date(task.showTime).toLocaleDateString('en-US', { timeZone: "Asia/Kolkata" })} </strong> at
                <strong>${new Date(task.showTime).toLocaleTimeString('en-US', { timezone: 'Asia/Kolkata' })}</strong>. </p>
      <p>It starts in approximately <strong>8 hours</strong> - make sure you're ready! </p> <br/>
      <p>Enjoy the show!<br />- jusTickets Team</p>
    </div>`
        }))
      )
    })
    ...
    return {
      sent,
      failed,
      message: `Sent ${sent} reminderTasks(s), ${failed} failed.`
    }
  }
)
